...
(function() {
  ...
  var _contentObjectToHTML = function(contentObj) {
    if (contentObj.type === undefined) {
      if (typeof(window) !== "undefined" && window.displayParagraphHTML) {
        return contentObj;
      }
  ...
  var _paragraphsToHTML = function(paragraphs) {
    ...
    for (var i = 0; i < paragraphs.length; ++i) {
      ...
      case 'heading':
        ...
        break;
      case 'paragraph':
        var paragraphHTML = _contentToHTML(paragraph.content);
        if (typeof(window) !== "undefined" && window.displayParagraphHTML) {
          result.push(window.displayParagraphHTML(paragraphHTML));
        } else {
          result.push('<p>');
